...
import { Building2, Phone, Mail, Globe, MapPin, Clock, Calendar, Stethoscope, Star, Users, ImageIcon, Edit2 } from "lucide-react"
import Image from "next/image"
import Link from "next/link"
import EditOrganizationDialog from "@/components/pageComponents/organizationprofile/EditOrganizationDialog"
import Loading from "@/components/ui/loading"

export default function MyOrganizationProfile() {
		...
		<div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 flex items-center justify-center">
			<Card className="max-w-md mx-4 shadow-xl border-0">
				<CardContent className="p-8 text-center">
					...
					<p className="text-gray-600">{error || "Your organization profile could not be found."}</p>
				</CardContent>
			</Card>
		</div>
	)
				...
				<div className="relative container mx-auto px-4 py-12">
					<div className="flex flex-col md:flex-row items-center gap-8 max-w-4xl mx-auto">
						...
						<div className="flex gap-3">
							<Button className="bg-white/20 hover:bg-white/30 text-white border border-white/30 backdrop-blur-sm transition-all duration-300 px-6 py-2.5 rounded-xl shadow-lg hover:shadow-xl"
					...
					<div className="space-y-6">
						...
						<Card className="shadow-lg border-0 bg-white/90 backdrop-blur-sm sticky top-6">
							<CardContent className="p-6">
								<Button
									onClick={() => setIsEditDialogOpen(true)}
									className="w-full bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white font-semibold py-4 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 mb-3"
								>
									<Edit2 className="w-5 h-5 mr-3" />
									Edit Profile
								</Button>
								<Link href={`/organizations/profile/${organization._id}`}>
									<Button variant="outline" className="w-full py-4 rounded-xl">
										View Public Profile
									</Button>
								</Link>
								{/* link with appointments */}
								<Link href={`/organizations/my/appointments`}>
									<Button variant="outline" className="w-full py-4 rounded-xl mt-3">
										<Calendar className="w-4 h-4 mr-2" />
										Appointments
									</Button>
								</Link>
								{/* link with afiliations */}
								<Link href={`/organizations/my/affiliations`}>
									<Button variant="outline" className="w-full py-4 rounded-xl mt-3">
										View Affiliations
									</Button>
								</Link>
