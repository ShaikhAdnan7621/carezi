...
import OrganizationTimePickerDialog from '@/components/appointments/organization/OrganizationTimePickerDialog';
import { Card, CardContent } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { Calendar, CheckCircle2, AlertCircle, Users, TrendingUp, ChevronLeft, ChevronRight, Plus, Building2 } from 'lucide-react';
...
export default function OrganizationAppointmentsPage() {
	...
	useEffect(() => {
		if (organization && professionals.length > 0) {
			fetchAppointments();
			fetchStats();
		}
	}, [currentViewDate, organization, professionals]);

	const fetchData = async () => {
		try {
			const [orgRes, profsRes] = await Promise.all([
				axios.get('/api/organization/profile'),
				axios.get('/api/affiliation/organization/my')
			]);

			setOrganization(orgRes.data);

			const approvedAffiliations = profsRes.data.data?.filter(
				aff => aff.status === 'approved'
			) || [];

			setProfessionals(approvedAffiliations.map(aff => aff.professionalId));
		} catch (error) {
	...
	const fetchStats = async () => {
		...
		try {
			...
			const stats = {
				requested: appointments.filter(apt => apt.status === 'requested').length,
			...
			<div className="min-h-screen bg-gray-50/30 flex items-center justify-center">
				<Card className="max-w-md mx-4 shadow-xl border-0">
					<CardContent className="p-8 text-center">
						...
						<h2 className="text-xl font-semibold text-gray-800 mb-2">Organization Not Found</h2>
				...
				<Card className="max-w-md mx-4 shadow-xl border-0">
					<CardContent className="p-8 text-center">
						...
						<p className="text-gray-600 mb-4">
							You need approved professional affiliations to manage appointments.
						</p>
						<Button asChild className="bg-emerald-600 hover:bg-emerald-700">
							<a href="/organizations/my/affiliations">View Affiliations</a>
						</Button>
					</CardContent>
				</Card>
			</div>
			...
			<div className="max-w-7xl mx-auto sm:px-6 lg:px-8 py-4 sm:py-8">
					...
					<div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-3">
						<div>
							...
							<p className="text-sm sm:text-base text-gray-600">
								Manage appointments for {organization.name}
							</p>
						</div>
									...
									{dateRange.map((date, index) => {
										...
										return (
											<Button
												...
												className={`h-13 lg:h-14 p-2 sm:p-2 flex flex-col items-center justify-start relative text-xs transition-all border shadow-sm ${isSelected ? 'bg-gradient-to-br from-green-500 to-green-600 text-white hover:from-green-600 hover:to-green-700 shadow-lg ring-2 ring-green-400 ring-opacity-50 border-green-600' :
													isToday ? 'bg-gradient-to-br from-green-50 to-green-100 text-green-800 border-2 border-green-400 hover:from-green-100 hover:to-green-200 shadow-md' :
														!isCurrentMonth ? 'text-gray-400 bg-gradient-to-br from-gray-50 to-gray-100 opacity-60 border-gray-200' :
															isPastDate ? 'text-gray-500 bg-gradient-to-br from-gray-100 to-gray-200 opacity-75 border-gray-300' : 'hover:bg-gradient-to-br hover:from-gray-50 hover:to-gray-100 border-gray-200 hover:border-gray-300 hover:shadow-md'
													}`}
											>
				...
				{showTimePickerDialog && (
					<OrganizationTimePickerDialog
						isOpen={showTimePickerDialog}
						onClose={() => setShowTimePickerDialog(false)}
